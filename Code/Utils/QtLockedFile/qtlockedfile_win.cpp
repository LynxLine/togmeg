...
bool QtLockedFile::lock(LockMode mode, bool block)
{
    ...
    if (mode == ReadLock) {
        ...
        DWORD res = WaitForSingleObject(m_mutex_hnd, block ? LOCK_WAIT_MSECS : 0);
        if (res == WAIT_TIMEOUT)
            return false;
        if (res == WAIT_FAILED) {
    ...
    for (int i = 0; i < decrement; ++i) {
        DWORD res = WaitForSingleObject(m_semaphore_hnd, block ? LOCK_WAIT_MSECS : 0);
        if (res == WAIT_TIMEOUT) {
            if (i) {
